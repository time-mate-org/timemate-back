...
async def create_appointment(
    ...
    appointment_start_time = datetime.fromisoformat(appointment_payload.start_time)
    appointment_end_time = get_appointment_end_time(appointment_start_time, service)

    check_concurrent_appointment_conflicts(
        session, client, professional, appointment_start_time, appointment_end_time
    )
...
async def update_appointment(
    ...
    if appointment_payload.start_time or appointment_payload.service_id:
        new_appointment_start_time = (
            datetime.fromisoformat(appointment_payload.start_time)
            if appointment_payload.start_time
            else db_appointment.start_time
        )
...
def get_appointment_end_time(start_time: datetime, service: Service):
    appointment_end_time = start_time + timedelta(
        minutes=service.estimated_time, microseconds=-1
    )
...
def check_concurrent_appointment_conflicts(
    ...
    same_professional_and_same_hour_appointments = list(
        filter(
            lambda appt: appt.professional_id == professional.id, same_hour_appointments
        )
    )
