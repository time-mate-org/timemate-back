...
def create_appointment(
    session: Session, start_time: datetime, professional_id: int = 1, client_id: int = 1
) -> Appointment:
...
class TestAppointmentBusinessRules:
    def test_create_appointment_professional_conflict_overlapping_start(
        ...
        """
        # Configuração
        _, professional_ent, service_ent = create_basic_entities(db_session)
    ...
    def test_create_appointment_professional_conflict_overlapping_end(
        self, client: TestClient, db_session: Session
    ):
        """
    ...
    def test_update_appointment_professional_conflict_overlapping_start(
        self, client: TestClient, db_session: Session
    ):
        """
        ...
        original_appointment = create_appointment(db_session, NOW.replace(minute=0))
        create_appointment(db_session, NOW.replace(minute=30), client_id=2)
        ...
        response = client.put(
            f"{BASE_URL}update/{original_appointment.id}", json=update_payload
        )
        log_test(
    ...
    def test_update_appointment_professional_conflict_overlapping_end(
        self, client: TestClient, db_session: Session
    ):
        """
        Testa atualização que causa conflito com agendamento existente do mesmo profissional
        ...
        response = client.put(
            f"{BASE_URL}update/{appointment_to_update.id}", json=update_payload
        )
        log_test(
    ...
    def test_create_appointment_client_conflict_overlapping_start(
        self, client: TestClient, db_session: Session
    ):
        """
        ...
        Novo: 14:15-14:45 → Deve conflitar
        """
        client_ent, _, service_ent = create_basic_entities(db_session)
    ...
    def test_create_appointment_client_conflict_overlapping_end(
        self, client: TestClient, db_session: Session
    ):
        """
    ...
    def test_update_appointment_client_conflict_overlapping_start(
        self, client: TestClient, db_session: Session
    ):
        """
        ...
        # Cria dois agendamentos iniciais
        create_appointment(db_session, NOW.replace(minute=30), professional_id=2)
        appointment_to_update = create_appointment(
            db_session,
            NOW.replace(minute=0),
        )

        update_payload = {"start_time": (NOW.replace(minute=45)).isoformat()}
...
class TestEdgeCases:
    def test_appointment_creation_after(self, client: TestClient, db_session: Session):
        ...
        """
        client_ent, professional_ent, service_ent = create_basic_entities(db_session)
        create_appointment(db_session, NOW.replace(minute=0))
        ...
        response = client.post(f"{BASE_URL}create/", json=payload)
        log_test(
            self.test_appointment_creation_after.__name__, "response", response.json()
        )
    ...
    def test_appointment_creation_before(self, client: TestClient, db_session: Session):
        ...
        response = client.post(f"{BASE_URL}create/", json=payload)
        log_test(
            self.test_appointment_creation_before.__name__, "response", response.json()
        )
    ...
    def test_appointment_update_before(self, client: TestClient, db_session: Session):
        ...
        response = client.put(f"{BASE_URL}update/{appointment.id}", json=payload)
        log_test(
            self.test_appointment_update_before.__name__, "response", response.json()
        )
...
class TestValidationRules:
    ...
    def test_create_appointment_end_time_calculus(
        self, client: TestClient, db_session: Session
    ):
        """
        ...
        assert (
            response_json["end_time"]
            == (
                NOW + timedelta(minutes=service_ent.estimated_time, microseconds=-1)
            ).isoformat()
        )

    def test_update_appointment_end_time_calculus(
        self, client: TestClient, db_session: Session
    ):
        """
        ...
        payload = {"service_id": new_service.id}
        ...
        assert (
            response_json["end_time"]
            == (
                NOW + timedelta(minutes=new_service.estimated_time, microseconds=-1)
            ).isoformat()
        )
    ...
    def test_create_invalid_date(self, client: TestClient, db_session: Session):
        ...
        response = client.post(f"{BASE_URL}create/", json=payload)
        response_json = response.json()
        log_test(self.test_create_invalid_date.__name__, "response", response_json)
